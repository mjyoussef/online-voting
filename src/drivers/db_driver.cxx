...
std::vector<VoteRow> DBDriver::all_votes() {
  ...
  std::string find_query = "SELECT vote, zkp, signature FROM vote";
...
VoteRow DBDriver::find_vote(Vote_Struct vote_s) {
  ...
  std::string find_query =
      "SELECT vote, zkp, signature FROM vote WHERE vote = ?";
...
VoteRow DBDriver::insert_vote(VoteRow vote) {
  ...
  sqlite3_bind_blob(stmt, 3, sign_str.c_str(), sign_str.length(),
                    SQLITE_STATIC);
